	...
	if pPlayer:IsAlive() and pPlayer:GetCivilizationType() == iUruguayCiv then
		...
		if iParty == 0 then
			...
			for pCity in pPlayer:Cities() do
				...
				--Hard nerf part, causes Blancos to build a round of military units every *30 turns.
				if (Game.GetElapsedGameTurns() + 15) % (frequency * 3) == 0 and Game.GetElapsedGameTurns() >= frequency * 10 then
						local iThingToProduce = load(pPlayer, sThingToProduce .. iCity);
						if iThingToProduce == nil then 
							iThingToProduce = -1; 
						end
...
function GetShouldPurchaseSettlers(iPlayer)
	--Nerf 1 (no purchasing settlers after T80 on Standard)
	local iTurns = 30;
		if Game.GetGameSpeedType() < 4 then 
			iTurns = vSpeed[Game.GetGameSpeedType()];
		end
	if Game.GetGameTurn() > iTurns * 3 then
		--print("No settlers for you!");
		return false;
	end 
	...
	--Hard nerf 2, makes AI severely limit built settlers
	local bRet = iTotalSettlers <= 1 + (iBuildSettlerLimit - 1) / 3.55 and not ((iPlayerCities + iTotalSettlers > iLargestCivCities  * 1.5 or iPlayerCities + iTotalSettlers > iLargestCivCities + 5) and iPlayerCities + iTotalSettlers >= iBuildSettlerLimit * 1.5);
	local sRet = "";
	if bRet then
		sRet = "true";
	else
	...
	local sCity = "";
	if ((iPlayerCities + iTotalSettlers > iLargestCivCities * 1.5 or iPlayerCities + iTotalSettlers > iLargestCivCities + 5) and iPlayerCities + iTotalSettlers >= iBuildSettlerLimit * 1.5) then
		sCity = "true";
	else
		sCity = "false";
	end
...
function GoldBuyDown(iPlayer, iParty, iGoldThreshold)
	...
	for pCity in pPlayer:Cities() do
		...
		--Nerf 2 (Limit unit purchase frequency)
		local iThreshold = 2;
		if iParty == 1 then
			iThreshold = 4;
		end
		...
		if iRand < iThreshold then
			...
			if iUnitOptions > 0 then
				...
				--Nerf 3 (artificially increase gold cost by 25% to limit just how many units Uruguay spams)
				local iCost = pCity:GetUnitPurchaseCost(validUnits[iRandB].ID) * 1.25;
			...
			if iBuildingsOptions > 0 then
				...
				--Nerf 3 (artificially increase gold cost by 25% to limit just how many units Uruguay spams)
				local iCost = pCity:GetBuildingPurchaseCost(validBuildings[iRandB].ID) * 1.25;
				local iGold = pPlayer:GetGold();
				if iGold - iCost > 0 and iCost >= 0 then
...
function UruguayPurchaseOverride(iPlayer) 
	...
	if pPlayer:IsAlive() and pPlayer:GetCivilizationType() == iUruguayCiv and not pPlayer:IsHuman() then
		...
		if iGold > 5000 then
			...
			print("Calling GoldBuyDown. JK, that got nerfed!");
			--Hard Nerf 1 (Disables GoldBuyDown)
...
function UruguayPurchaseAuto(iPlayer)
	...
	if pPlayer:IsAlive() and pPlayer:GetCivilizationType() == iUruguayCiv and not pPlayer:IsHuman() then
		...
		if Game.GetElapsedGameTurns() % frequency == 0 then
			...
			--Hard nerf 1: Disables routine buying
...
function UruguayBuildOverride(iPlayer)
	...
	if pPlayer:IsAlive() and pPlayer:GetCivilizationType() == iUruguayCiv and not pPlayer:IsHuman() then
		...
		--[[if Game.GetElapsedGameTurns() < frequency * 10 then
			--Easiest nerf ever. Just delete the whole early-game script by exiting the method if before T100 (Standard).
			return;
			...
			for pCity in pPlayer:Cities() do
				...
				elseif pCity:CanTrain(GameInfoTypes.UNIT_SETTLER) and GetShouldPurchaseSettlers(iPlayer) and iSettlers <= 3 and iCityCount < 6 and Game.GetElapsedGameTurns() > frequency * 4 then
					--Hard nerf component, makes Uruguay build extra settlers with production rather than buying them. Limited to first 6 cities building 3 settlers each as a sanity check.
					--print("Checking if can build extra settler");
					if iProd >= pPlayer:GetUnitProductionNeeded(GameInfoTypes.UNIT_SETTLER) / (1 + pCity:GetUnitProductionModifier(GameInfoTypes.UNIT_SETTLER) / 100) then
						print("Building extra settler");
						iSettlers = iSettlers + 1;
						pPlayer:InitUnit(GameInfoTypes.UNIT_SETTLER, pCity:GetX(), pCity:GetY());
						iProd = iProd - (pPlayer:GetUnitProductionNeeded(GameInfoTypes.UNIT_SETTLER) / (1 + pCity:GetUnitProductionModifier(GameInfoTypes.UNIT_SETTLER) / 100));
					end
